...
(function() {
  ...
  function startLoop(direction) {
    ...
    // Wait for runCheck to finish, then start loop timers
    ...
    if (checkPromise && typeof checkPromise.then === 'function') {
      checkPromise.then(function() {
        ...
        log('Initial check failed: ' + err.message + ' — starting loop anyway', 'warn');
        startTimers();
      });
    } else {
  ...
  function createUI() {
    ...
    ui.id = IDS.CONTAINER;
    ui.style.cssText = 'background:#1e1b4b;border:1px solid #312e81;border-radius:12px;padding:12px;margin:8px 0;font-family:monospace;font-size:12px;color:#e0e7ff;';
    ...
    checkBtn.onclick = function() {
      // v7.14.1: Only allow manual check when loop is not running OR countdown is close to trigger (<=10s)
      if (state.running && state.countdown > 10) {
        log('Check blocked: countdown=' + state.countdown + 's (must be <=10s or loop stopped)', 'warn');
